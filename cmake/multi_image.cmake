...
#
function(get_child_image_cmake_args child_image output_cmake_args)
  ...
  set(conf_dir ${APPLICATION_CONFIG_DIR}/child_image/${child_image})
  if (NOT ${child_image}_CONF_FILE)
    ncs_file(CONF_FILES ${conf_dir}
             BOARD ${ACI_BOARD}
             # Child image always uses the same revision as parent board.
             BOARD_REVISION ${BOARD_REVISION}
             KCONF ${child_image}_CONF_FILE
             DTS ${child_image}_DTC_OVERLAY_FILE
    ...
    if (DEFINED CONF_FILE_BUILD_TYPE)
      set(child_image_conf_fragment ${fragments_dir}/${child_image}_${CONF_FILE_BUILD_TYPE}.conf)
    else()
      set(child_image_conf_fragment ${fragments_dir}/${child_image}.conf)
    endif()
    if (EXISTS ${child_image_conf_fragment})
      add_overlay_config(${child_image} ${child_image_conf_fragment})
    endif()

    # Check for overlay named <child_image>.overlay.
    set(child_image_dts_overlay ${fragments_dir}/${child_image}.overlay)
    if (EXISTS ${child_image_dts_overlay})
      add_overlay_dts(${child_image} ${child_image_dts_overlay})
    endif()
    if(DEFINED ${child_image}_CONF_FILE)
      set(${child_image}_CONF_FILE ${${child_image}_CONF_FILE} CACHE STRING
        "Default ${child_image} configuration file"
      )
    endif()
  endif()
  ...
  foreach(var_name
    ...
    string(REGEX MATCH "^${child_image}_(.+)" unused_out_var ${var_name})
  ...
  unset(${output_cmake_args} PARENT_SCOPE)
  set(${output_cmake_args} "${child_image_cmake_args}" PARENT_SCOPE)
endfunction()

# Adds an S1 variant of an image to the build.
#
# Required arguments are:
# SOURCE_DIR - The source dir of the base image.
#
# Optional argumnets are:
...
function(add_s1_variant)
  ...
  if (NOT ASV_SOURCE_DIR)
    message(FATAL_ERROR "Missing parameter, required: SOURCE_DIR")
  endif()
...
function(add_image_variant)
  ...
  if (NOT AIV_IMAGE_NAME OR NOT AIV_SOURCE_DIR OR NOT AIV_EXTRA_CONFIG)
    message(FATAL_ERROR "Missing parameter, required: IMAGE_NAME SOURCE_DIR EXTRA_CONFIG ")
  endif()
  ...
  if (AIV_BASE_IMAGE)
    ...
    foreach(app_var_name ${application_vars})
      ...
      string(REPLACE \; \\\\\; val "${${app_var_name}}")  # Ensure correct list handling
      list(APPEND base_image_configuration -D${app_var_name}:${var_type}=${val})
    endforeach()
  endif()
...
function(add_child_image_from_source)
  ...
  if (PRE_DEFINED_IMAGE_CMAKE_ARGS_${ACI_NAME})
    ...
    get_child_image_cmake_args(${ACI_NAME} image_cmake_args)
  endif()
