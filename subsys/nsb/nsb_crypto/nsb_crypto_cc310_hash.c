...
#include "nsb_crypto_cc310_common.h"
...
BUILD_ASSERT_MSG(SHA256_CTX_SIZE >= sizeof(nrf_cc310_nsb_hash_context_sha256_t), \
		"nrf_cc310_nsb_hash_context_sha256_t can no longer fit inside " \
		"nsb_sha256_ctx_t.");
...
{
	return cc310_nsb_init();
}


int nsb_sha256_init(nrf_cc310_nsb_hash_context_sha256_t * const ctx)
{
	CRYSError_t retval = nrf_cc310_nsb_hash_sha256_init(ctx);
...
static int hash_blocks(nrf_cc310_nsb_hash_context_sha256_t *const ctx,
		...
		u32_t *buffer)
{
	...
	for (u32_t i = 0; i < data_len; i += max_chunk_len) {
		...
		retval = nrf_cc310_nsb_hash_sha256_update(ctx, source, chunk_len);
	...
	cc310_nsb_backend_disable();

	return retval;
}


static inline int hash_blocks_stack(nrf_cc310_nsb_hash_context_sha256_t *const ctx,
...
static int sha256_update(nrf_cc310_nsb_hash_context_sha256_t *const ctx,
...
int nsb_sha256_update(nrf_cc310_nsb_hash_context_sha256_t *ctx,
		const u8_t * data, u32_t data_len)
{
	return sha256_update(ctx, data, data_len, true);
}

int nsb_sha256_finalize(nrf_cc310_nsb_hash_context_sha256_t *ctx, u8_t *output)
{
	cc310_nsb_backend_enable();
	CRYSError_t retval = nrf_cc310_nsb_hash_sha256_finalize(ctx,
			(nrf_cc310_nsb_hash_digest_sha256_t *) output);
	cc310_nsb_backend_disable();
...
{
	nrf_cc310_nsb_hash_context_sha256_t ctx;
	int retval;

	retval = nsb_sha256_init(&ctx);
	if (retval != 0) {
		return retval;
	}
	...
	retval = nsb_sha256_finalize(&ctx, hash);
	return retval;
}
