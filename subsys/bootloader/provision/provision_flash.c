...
int public_key_data_read(u32_t key_idx, u32_t *p_buf, size_t buf_size)
{
	...
	if (buf_size < CONFIG_NSB_PUBLIC_KEY_HASH_LEN) {
		return -ENOMEM;
	}
	...
	p_key = &(p_provision_data->pkd[key_idx *
			CONFIG_NSB_PUBLIC_KEY_HASH_LEN]);
	 ...
	 */
	BUILD_ASSERT(CONFIG_NSB_PUBLIC_KEY_HASH_LEN % 4 == 0);
	BUILD_ASSERT(offsetof(provision_flash_t, pkd) % 4 == 0);
	__ASSERT(((u32_t)p_key % 4 == 0), "Key address is not word aligned");
#endif /* CONFIG_SOC_NRF9160 */

	for (size_t i = 0; i < CONFIG_NSB_PUBLIC_KEY_HASH_LEN/4; i++) {
		p_buf[i] = p_key[i];
	}

	return CONFIG_NSB_PUBLIC_KEY_HASH_LEN;
}
