...
BUILD_ASSERT_MSG(CONFIG_NSB_PUBLIC_KEY_HASH_LEN <= CONFIG_NSB_HASH_LEN,
...
{
	u8_t hash[CONFIG_NSB_HASH_LEN];

	__ASSERT(hash_len <= CONFIG_NSB_HASH_LEN, "truncated hash length too long.");
...
{
	u8_t hash1[CONFIG_NSB_HASH_LEN];
	u8_t hash2[CONFIG_NSB_HASH_LEN];
	...
	retval = get_hash(hash2, hash1, CONFIG_NSB_HASH_LEN, external);
	if (retval != 0) {
		return retval;
	}

	return nsb_secp256r1_validate(hash2, CONFIG_NSB_HASH_LEN, public_key, signature);
}
		...
		const u32_t firmware_len, bool external)
{
	...
	int retval = verify_truncated_hash(public_key, CONFIG_NSB_PUBLIC_KEY_LEN,
			public_key_hash, CONFIG_NSB_PUBLIC_KEY_HASH_LEN, external);
...
int nsb_root_of_trust_verify_external(
			const u8_t *public_key, const u8_t *public_key_hash,
			const u8_t *signature, const u8_t *firmware,
			const u32_t firmware_len)
{
...
{
	return verify_truncated_hash(data, data_len, expected, CONFIG_NSB_HASH_LEN, true);
}

__ext_abi(struct nsb_rot_verify_abi, nsb_rot_verify_abi) = {
	.header = ABI_INFO_INIT(BL_ROT_VERIFY_ABI_ID, BL_ROT_VERIFY_ABI_FLAGS,
				BL_ROT_VERIFY_ABI_VER, sizeof(struct nsb_rot_verify_abi)),
	.abi = {
		.nsb_root_of_trust_verify = nsb_root_of_trust_verify_external,
	}
};

__ext_abi(struct nsb_sha256_abi, nsb_sha256_abi) = {
	.header = ABI_INFO_INIT(BL_SHA256_ABI_ID, BL_SHA256_ABI_FLAGS,
				BL_SHA256_ABI_VER, sizeof(struct nsb_sha256_abi)),
	.abi = {
		.nsb_sha256_init = nsb_sha256_init,
		.nsb_sha256_update = nsb_sha256_update,
		.nsb_sha256_finalize = nsb_sha256_finalize,
		.nsb_sha256_verify = nsb_sha256_verify,
		.nsb_sha256_ctx_size = SHA256_CTX_SIZE,
	},
};

__ext_abi(struct nsb_secp256r1_abi, nsb_secp256r1_abi) = {
	.header = ABI_INFO_INIT(BL_SECP256R1_ABI_ID, BL_SECP256R1_ABI_FLAGS,
				BL_SECP256R1_ABI_VER, sizeof(struct nsb_secp256r1_abi)),
	.abi = {
		.nsb_secp256r1_validate = nsb_secp256r1_validate,
	},
};
