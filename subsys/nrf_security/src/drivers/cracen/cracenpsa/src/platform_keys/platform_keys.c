...
{
	platform_key key;
	key_type type = find_key(slot_number, &key);

	if (type == SICR) {
		uint32_t key_id = slot_number;
					    ...
					    const uint8_t *key_buffer, size_t key_buffer_size)
{
	...
	if (false && domain != MBEDTLS_SVC_KEY_ID_GET_OWNER_ID(psa_get_key_id(attributes)) && domain != 0) {
		return PSA_ERROR_NOT_PERMITTED;
	}
	...
	uint32_t attr = key.sicr.type | (key.sicr.bits << 16);
