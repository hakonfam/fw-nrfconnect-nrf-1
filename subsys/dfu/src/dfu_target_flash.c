...
/* TODO take length as an argument, so it can be given to the callback */
static int write_buf_to_flash(void)
{
...
#ifdef CONFIG_DFU_TARGET_READ_BACK
	/* write_buf has now been dumped to flash, re-use it to allow
	 * operations like a hash to take place, verifying the write 
	 */
	...
	callback(write_buf, layout->pages_size);
#endif
...
int dfu_target_flash_cfg(const char *dev_name, size_t start, size_t end,
			 ...
			 dfu_target_callback_t callback_in)
{
	size_t total_size;
	size_t layout_size;
