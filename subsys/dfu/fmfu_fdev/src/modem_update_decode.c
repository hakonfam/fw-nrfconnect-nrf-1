...
static bool decode_Segment(cbor_state_t *state, Segment_t *result)
{
	cbor_print("%s\n", __func__);
...
static bool decode_Manifest(cbor_state_t *state, Manifest_t *result)
{
	cbor_print("%s\n", __func__);
	bool int_res;
...
static bool decode_Segments(cbor_state_t *state, Segments_t *result)
{
	cbor_print("%s\n", __func__);
	bool int_res;
...
static bool decode_Sig_structure1(cbor_state_t *state, Sig_structure1_t *result)
{
	cbor_print("%s\n", __func__);
...
static bool decode_Wrapper(cbor_state_t *state, COSE_Sign1_Manifest_t *result)
{
	cbor_print("%s\n", __func__);
...
type_test_decode_Wrapper(COSE_Sign1_Manifest_t *result)
{
...
bool cbor_decode_Wrapper(const uint8_t *payload, size_t payload_len,
			 COSE_Sign1_Manifest_t *result, size_t *payload_len_out)
{
...
__attribute__((unused)) static bool
type_test_decode_Sig_structure1(Sig_structure1_t *result)
{
	/* This function should not be called, it is present only to test that the
	 * types of the function and struct match, since this information is lost
	 * with the casts in the entry funciton.
...
__attribute__((unused)) static bool
type_test_decode_Segments(Segments_t *result)
{
	/* This function should not be called, it is present only to test that the
	 * types of the function and struct match, since this information is lost
	 * with the casts in the entry funciton.
