...
static void update_transfer_start(struct k_work *unused)
{
	...
	retval = fota_download_start(host_get(), file_get(), SEC_TAG, apn, 0);
	if (retval != 0) {
		sw0_irq_enable();
		       ...
		       bool (*two_leds_in)(void))
{
	...
	if (finished_cb_in == NULL || host_get_in == NULL ||
	    file_get_in == NULL || two_leds_in == NULL) {
		return -EINVAL;
	}
