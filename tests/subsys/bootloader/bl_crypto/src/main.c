...
void test_ecdsa_verify(void)
{
	...
	/* Success */
	int retval = nsb_secp256r1_validate(hash_sha256, hash_len, pub_concat, sig_concat);
	zassert_equal(0, retval, "retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(const_hash_sha256, hash_len, pub_concat, sig_concat);
	zassert_equal(0, retval, "retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(hash_sha256, hash_len, const_pub_concat, sig_concat);
	zassert_equal(0, retval, "retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(hash_sha256, hash_len, pub_concat, const_sig_concat);
	zassert_equal(0, retval, "retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(hash_sha256, hash_len, const_pub_concat, const_sig_concat);
	zassert_equal(0, retval, "retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(const_hash_sha256, hash_len, const_pub_concat, const_sig_concat);
	zassert_equal(0, retval, "retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(const_hash_sha256, hash_len, const_pub_concat, const_sig_concat);
	zassert_equal(0, retval, "retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(image_fw_hash, hash_len, image_public_key, image_gen_sig);
	zassert_equal(0, retval, "gen sig failed retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(image_fw_hash, hash_len, image_public_key, image_fw_sig);
	...
	retval = nsb_secp256r1_validate(const_fw_hash, hash_len, image_public_key, image_fw_sig);
	zassert_equal(0, retval, "const_fw retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(image_fw_hash, hash_len, const_public_key, image_fw_sig);
	zassert_equal(0, retval, "const_pubk retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(image_fw_hash, hash_len, image_public_key, const_fw_sig);
	zassert_equal(0, retval, "const_sig retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(image_fw_hash, hash_len, image_public_key, const_fw_sig);
	zassert_equal(0, retval, "const_sig retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(image_fw_hash, hash_len, image_public_key, const_gen_sig);
	zassert_equal(0, retval, "const_gen_sig retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(image_fw_hash, hash_len, const_public_key, const_fw_sig);
	zassert_equal(0, retval, "const_sig const_pk retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(const_fw_hash, hash_len, const_public_key, const_fw_sig);
	zassert_equal(0, retval, "const_sig const_pk const_fw retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(const_fw_hash, hash_len, image_public_key, const_fw_sig);
	zassert_equal(0, retval, "const_sig const_fw retval: 0x%x", retval);

	retval = nsb_secp256r1_validate(const_fw_hash, hash_len, const_public_key, image_fw_sig);
	zassert_equal(0, retval, "const_pk const_fw retval: 0x%x", retval);

	/* pub key does not match */
	pub_concat[1]++;
	retval = nsb_secp256r1_validate(hash_sha256, hash_len, pub_concat, sig_concat);
	pub_concat[1]--;
	...
	retval = nsb_secp256r1_validate(hash_sha256, hash_len, pub_concat, const_sig_concat);
	zassert_equal(-ESIGINV, retval, "retval: 0x%x", retval);
	retval = nsb_secp256r1_validate(const_hash_sha256, hash_len, pub_concat, const_sig_concat);
	zassert_equal(-ESIGINV, retval, "retval: 0x%x", retval);
	pub_concat[1]--;
	...
	retval = nsb_secp256r1_validate(NULL, hash_len, pub_concat, sig_concat);
	zassert_equal(-EINVAL, retval, "retval: 0x%x", retval);

	/* Wrong hash LEN */
	retval = nsb_secp256r1_validate(hash_sha256, 24, pub_concat, sig_concat);
	zassert_equal(-EINVAL, retval, "retval: 0x%x", retval);

	/* Null pointer passed as public key */
	retval = nsb_secp256r1_validate(hash_sha256, hash_len, NULL, sig_concat);
	zassert_equal(-EINVAL, retval, "retval: 0x%x", retval);

	/* Null pointer passed as signature */
	retval = nsb_secp256r1_validate(hash_sha256, hash_len, pub_concat, NULL);
	zassert_equal(-EINVAL, retval, "retval: 0x%x", retval);
}
...
void test_sha256_string(const uint8_t * input, uint32_t input_len, const uint8_t * test_vector, bool eq)
{
	int rc = 0;
	uint8_t output[32] = {0};
	nsb_sha256_ctx_t ctx;
	static u32_t run_count = 0;

	rc = nsb_sha256_init(&ctx);
	rc = nsb_sha256_update(&ctx, input, input_len);
	rc = nsb_sha256_finalize(&ctx, output);
	...
	rc = nsb_sha256_verify(input, input_len, test_vector);

	int expected_rc = eq ? 0 : -EHASHINV;
	zassert_equal(expected_rc, rc, "nsb_sha256_verify returned %d instead of %d", rc, expected_rc);
...
void test_nsb_root_of_trust_verify(void)
{

	/* Success. */
	int retval = nsb_root_of_trust_verify(pk, pk_hash, sig, firmware,
			sizeof(firmware));

	zassert_equal(0, retval, "retval was %d", retval);

	retval = nsb_root_of_trust_verify(const_pk, const_pk_hash, const_sig, const_firmware,
	...
	pk[1]++;
	retval = nsb_root_of_trust_verify(pk, pk_hash, sig, firmware,
			sizeof(firmware));
	pk[1]--;
...
void test_main(void)
{
	ztest_test_suite(test_nsb_crypto,
			 ztest_unit_test(test_nsb_root_of_trust_verify),
			 ztest_unit_test(test_sha256),
			 ztest_unit_test(test_ecdsa_verify)
	);
	ztest_run_test_suite(test_nsb_crypto);
}
