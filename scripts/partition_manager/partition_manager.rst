...
RAM partition configuration
   RAM partitions are partitions located in the ``ram_primary`` region.
   A RAM partition is specified by having the partition name end with ``_ram``.
   If a partition name consists of an image name and the ending ``_ram``, it is used as a :ref:`pm_permanent_image_ram_partition` for the image.

   .. code-block:: yaml
      :caption: RAM partitions configuration

      # This ...
      some_permament_ram_block_used_for_logging:
         size: 0x1000
         region: ram_primary

      # ... is equivalent to
      some_permament_ram_block_used_for_logging_ram:
         size: 0x1000
