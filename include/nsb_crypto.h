...
#define ESIGINV  102
	...
	typedef u32_t nsb_sha256_ctx_t[SHA256_CTX_SIZE/4];
#endif
 ...
 * @return Any error code from @ref nsb_sha256_init, @ref nsb_sha256_update,
 *         @ref nsb_sha256_finalize, or @ref nsb_secp256r1_validate if something
 *         else went wrong.
 *
 * @remark No parameter can be NULL.
 */
EXT_ABI_FUNCTION(int, nsb_root_of_trust_verify, const u8_t *public_key,
 ...
 */
EXT_ABI_FUNCTION(int, nsb_sha256_init, nsb_sha256_ctx_t *ctx);
 ...
 */
EXT_ABI_FUNCTION(int, nsb_sha256_update, nsb_sha256_ctx_t *ctx,
					const u8_t *data,
					u32_t data_len);
 ...
 */
EXT_ABI_FUNCTION(int, nsb_sha256_finalize, nsb_sha256_ctx_t *ctx,
					  u8_t *output);
 ...
 * @return Any error code from @ref nsb_sha256_init, @ref nsb_sha256_update, or
 *         @ref nsb_sha256_finalize if something else went wrong.
 */
EXT_ABI_FUNCTION(int, nsb_sha256_verify, const u8_t *data,
					u32_t data_len,
					const u8_t *expected);
 ...
 */
EXT_ABI_FUNCTION(int, nsb_secp256r1_validate, const u8_t *hash,
					     u32_t hash_len,
					     const u8_t *signature,
					     const u8_t *public_key);

struct nsb_rot_verify_abi {
	struct fw_abi_info header;
	struct {
		nsb_root_of_trust_verify_t nsb_root_of_trust_verify;
	} abi;
};

struct nsb_sha256_abi {
	struct fw_abi_info header;
	struct {
		nsb_sha256_init_t nsb_sha256_init;
		nsb_sha256_update_t nsb_sha256_update;
		nsb_sha256_finalize_t nsb_sha256_finalize;
		nsb_sha256_verify_t nsb_sha256_verify;
		u32_t nsb_sha256_ctx_size;
	} abi;
};

struct nsb_secp256r1_abi {
	struct fw_abi_info header;
	struct {
		nsb_secp256r1_validate_t nsb_secp256r1_validate;
	} abi;
};
